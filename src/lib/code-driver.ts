...
export function parseCPP(code: string): MethodSignature | null {
    ...
    const [, returnType, methodName, argsRaw] = methodMatch;
    const args = argsRaw
        .split(",")
        .map((arg) => {
...
export function generateCPPDriver(sig: MethodSignature): string {
    let inputCode = "";
    const callArgs = sig.args.map((a) => a.name).join(", ");
...
export function parsePython(code: string): MethodSignature | null {
    ...
    const [, methodName, argsRaw] = methodMatch;
    const args = argsRaw
        .split(",")
        .map((arg) => {
...
export function generatePythonDriver(sig: MethodSignature): string {
    const callArgs = sig.args.map((_, i) => `_args[${i}]`).join(", ");
